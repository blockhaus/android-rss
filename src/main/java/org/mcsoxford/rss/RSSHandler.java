...
/**
 * Internal SAX handler to efficiently parse RSS feeds. Only a single thread must use this SAX handler.
 * 
 * @author Mr Horn
 */
class RSSHandler extends org.xml.sax.helpers.DefaultHandler {
	...
	/**
	 * Reference is never {@code null}. Visibility must be package-private to ensure efficiency of inner classes.
	 */
	final RSSFeed feed = new RSSFeed();

	/**
	 * Reference is {@code null} unless started to parse &lt;item&gt; element. Visibility must be package-private to
	...
	/**
	 * Closure to change fields in POJOs which store information about RSS elements which have only attributes.
	 */
	...
	/**
	 * Setter for RSS &lt;title&gt; elements inside a &lt;channel&gt; or an &lt;item&gt; element. The title of the RSS
	 * feed is set only if {@link #item} is {@code null}. Otherwise, the title of the RSS {@link #item} is set.
	 */
	...
	/**
	 * Setter for RSS &lt;description&gt; elements inside a &lt;channel&gt; or an &lt;item&gt; element. The title of the
	 * RSS feed is set only if {@link #item} is {@code null}. Otherwise, the title of the RSS {@link #item} is set.
	 */
	...
	/**
	 * Setter for an RSS &lt;content:encoded&gt; element inside an &lt;item&gt; element.
	 */
	...
	/**
	 * Setter for RSS &lt;link&gt; elements inside a &lt;channel&gt; or an &lt;item&gt; element. The title of the RSS
	 * feed is set only if {@link #item} is {@code null}. Otherwise, the title of the RSS {@link #item} is set.
	 */
	...
	/**
	 * Setter for RSS &lt;pubDate&gt; elements inside a &lt;channel&gt; or an &lt;item&gt; element. The title of the RSS
	 * feed is set only if {@link #item} is {@code null}. Otherwise, the title of the RSS {@link #item} is set.
	 */
	...
	/**
	 * Setter for one or multiple RSS &lt;category&gt; elements inside a &lt;channel&gt; or an &lt;item&gt; element. The
	 * title of the RSS feed is set only if {@link #item} is {@code null}. Otherwise, the title of the RSS {@link #item}
	 * is set.
	 */
...
//    <enclosure url="http://www.aez-wheels.com/67332_EN" length="182888" type="image/jpeg" />

	/**
	 * Blockhaus !
	 */
	private final Setter ADD_ENCLOSURE = new AttributeSetter() {
		...
		public void set(org.xml.sax.Attributes attributes) {
			...
			item.addThumbnail(new MediaThumbnail(android.net.Uri.parse(url), 0, 0));
		}

	};
	
	/**
	 * Setter for one or multiple RSS &lt;media:thumbnail&gt; elements inside an &lt;item&gt; element. The thumbnail
	 * element has only attributes. Both its height and width are optional. Invalid elements are ignored.
	 */
	 ...
	 * Blockhaus !
	 */
	private final Setter SET_VIDEO_RESTRICTED = new AttributeSetter() {

		@Override
		public void set(org.xml.sax.Attributes attributes) {
	        item.setIsRestrictedContent(true);
		}
	};
	...
	RSSHandler(RSSConfig config) {
		...
		setters.put("app:control", SET_VIDEO_RESTRICTED);
		setters.put("enclosure", ADD_ENCLOSURE);
	}
	...
	/**
	 * Identify the appropriate dispatcher which should be used to store XML data in a POJO. Unsupported RSS 2.0
	...
	/**
	 * Determines if the SAX parser is ready to receive data inside an XML element such as &lt;title&gt; or
	 * &lt;description&gt;.
	 * 
	 * @return boolean {@code true} if the SAX handler parses data inside an XML element, {@code false} otherwise
	 */
	boolean isBuffering() {
